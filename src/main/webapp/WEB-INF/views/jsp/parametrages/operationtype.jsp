...
<script type="text/javascript">
	...
	function save(){
		jQuery.ajax({
            type: "POST",
            url: contextPath+"/settings/saveoperationtype",
            cache: false,
            data: {
				id: jQuery('#id').val(),
            ...
            complete: function () {
        		jQuery('#table').bootstrapTable('refresh', {
                    url: contextPath + '/settings/operationstypesjson'
                });
            }
        });
	}
	...
	function edit(id){
		jQuery.ajax({
            type: "POST",
            url: contextPath+"/settings/loadoperationtype",
            cache: false,
            data: {id: id},
			success: function (response) {
            	if (response != null) {
	...
	function del(id){
		...
		if (r == true) {
			jQuery.ajax({
	            type: 'POST',
	            url: contextPath+'/settings/deleteoperationtype',
	            cache: false,
	            data: {id: id},
				success: function (response) {
	...
	function optionFormatter(id, row, index) {
		var option = '<a onclick="edit(\''+row.id+'\')" data-toggle="modal" href="#affconfirmmsg" title="Modifier Nature Mouvement [Flag : '+row.flag+' - Libelle : '+row.worded+' - Description : '+row.description+' ]"><span class="glyphicon glyphicon-file"></span></a>&nbsp;';
		option += '&nbsp;<a onclick="del(\''+row.id+'\')" data-toggle="modal" href="#affconfirmmsg" title="Suprimer Nature Mouvement [Flag : '+row.flag+' - Libelle : '+row.worded+' - Description : '+row.description+' ]"><span class="glyphicon glyphicon-trash"></span></a>';
        return option;
    }
</script>
...
<div class="widget">
	<div class="widgetbox">                        
	    <div class="headtitle">
	        ...
	        <h4 id="widgetTitle" class="widgettitle">Liste Natures Mouvements</h4>
	    </div>
	    <div id="tableWidget" class="widgetcontent">
	    	<table id="table" class="table table-info table-striped"
			       	...
			       	data-url="${pageContext.request.contextPath}/settings/operationstypesjson"
