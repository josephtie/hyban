...
<script type="text/javascript">
	...
	function loadDataToForm(id, bin, name, address, manager, phoneNumber, email){
		jQuery('#id').val(id);
		jQuery('#bin').val(bin);
	...
	function save(){
		jQuery.ajax({
            type: "POST",
            url: contextPath+"/settings/savebank",
            cache: false,
            data: {
				id: jQuery('#id').val(),
				bin: jQuery('#bin').val(),
            ...
            complete: function () {
        		jQuery('#table').bootstrapTable('refresh', {
                    url: contextPath + '/settings/banksjson'
                });
            }
        });
	}
	...
	function edit(id){
		jQuery.ajax({
            type: "POST",
            url: contextPath+"/settings/loadbank",
            cache: false,
            data: {id: id},
			success: function (response) {
            	if (response != null) {
            		loadDataToForm(response.id, response.bin, response.name, response.address, response.manager, response.phoneNumber, response.email);
            		widgetView('edit');
				} else {
	...
	function del(id){
		...
		if (r == true) {
			jQuery.ajax({
	            type: 'POST',
	            url: contextPath+'/settings/deletebank',
	            cache: false,
	            data: {id: id},
				success: function (response) {
	...
	function optionFormatter(id, row, index) {
		var option = '<a onclick="edit(\''+row.id+'\')" data-toggle="modal" href="#" title="Modifier Banque [BIN : '+row.bin+' - Nom : '+row.name+' ]"><span class="glyphicon glyphicon-file"></span></a>&nbsp;';
		option += '&nbsp;<a onclick="del(\''+row.id+'\')" data-toggle="modal" href="#" title="Suprimer Banque [BIN : '+row.bin+' - Nom : '+row.name+' ]"><span class="glyphicon glyphicon-trash"></span></a>';
        return option;
    }
</script>
...
<div class="widget">
	<div class="widgetbox">                        
	    <div class="headtitle">
	        ...
	        <h4 id="widgetTitle" class="widgettitle">Liste Banques</h4>
	    </div>
	    <div id="tableWidget" class="widgetcontent">
	    	<table id="table" class="table table-info table-striped"
			       	...
			       	data-url="${pageContext.request.contextPath}/settings/banksjson"
			    ...
			    <thead>
			        <tr>
			        	<th data-field="bin" data-align="center" data-sortable="true">BIN</th>
			        	...
			        	<th data-field="manager" data-align="left" data-sortable="true">Directeur</th>
	    ...
	    <div id="formWidget" class="widgetcontent">
	            ...
	            <div class="row">
                    <div class="form-group col-md-4">
                        ...
                        <input type="text" class="form-control input-default" id="bin" placeholder="BIN">
                    </div>
                    <div class="form-group col-md-4">
