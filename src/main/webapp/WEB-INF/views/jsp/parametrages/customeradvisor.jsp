<%@page contentType="text/html"%>
<%@taglib uri="http://www.springframework.org/tags" prefix="spring"%>
<%@taglib uri="http://www.springframework.org/tags/form" prefix="form"%>
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>

<script type="text/javascript">
	var contextPath = "<%=request.getContextPath() %>";
	
	jQuery(document).ready(function() {
		jQuery('#formWidget').hide();
		jQuery('#tableWidget').show();
		jQuery("#birthday").datepicker({
			dateFormat: 'dd/mm/yy',
	        showOtherMonths:true
	    });
		
		loadBranchs();
	});
	
	jQuery('#reset').click(function(){
		loadDataToForm(jQuery('#id').val(), null, null, null);
	});
	
	function widgetView(view){
		if(view == 'list'){
			jQuery('#formWidget').hide('slow');
			jQuery('#tableWidget').show('slow');
			jQuery('#widgetTitle').html('Liste Conseillers Clients');
		} else if(view == 'new'){
			jQuery('#flag').attr('readOnly', false);
    		loadDataToForm(null, null, null, null);
			jQuery('#widgetTitle').html('Nouvelle Conseiller Client');
			jQuery('#formWidget').show('slow');
			jQuery('#tableWidget').hide('slow');
		} else{
			jQuery('#flag').attr('readOnly', true);
			jQuery('#widgetTitle').html('Modifier Conseiller Client');
			jQuery('#formWidget').show('slow');
			jQuery('#tableWidget').hide('slow');
		}
		return false;
	}
	
	function loadBranchs(){
		jQuery.ajax({
            type: "POST",
            url: contextPath+"/settings/loadbranchs",
            cache: false,
            success: function (response) {
            	if (response != null) {
					tabledata = '';
	        		for(i =0 ; i < response.length ; i++){
	        			tabledata += '<option value="'+response[i].id+'" >'+response[i].name+'</option>';
	        		}
	        		//tabledata += "";
		    	  	jQuery('#idBranch').html(tabledata);
		    	  	jQuery("#idBranch").trigger("liszt:updated");
		    	  	jQuery('#idBranch option:selected').val()
				} else {
					alert('Failure! An error has occurred!');
				}
            },
            error: function () {
                
            },
        });
	}
	
	function loadDataToForm(id, idBranch, name, birthday, phoneNumber, address, email, password){
		jQuery('#id').val(id);
		if(idBranch == null){
			jQuery('#idBranch').trigger('liszt:updated');
    	  	jQuery('#idBranch option:selected').val()
		} else{
			jQuery('#idBranch').val(idBranch);
			jQuery("#idBranch").val(idBranch).trigger('change');
			jQuery('#idBranch').trigger('liszt:updated');
		}
		jQuery('#name').val(name);
		jQuery('#birthday').val(birthday);
		jQuery('#phoneNumber').val(phoneNumber);
		jQuery('#address').val(address);
		jQuery('#email').val(email);
		jQuery('#password').val(null);
	}
	
	function save(){
		jQuery.ajax({
            type: "POST",
            url: contextPath+"/settings/savecustomeradvisor",
            cache: false,
            data: {
				id: jQuery('#id').val(),
				idbranch: jQuery('#idBranch').val(),
				name: jQuery('#name').val(),
				birthday: jQuery('#birthday').val(),
				address: jQuery('#address').val(),
				phonenumber: jQuery('#phoneNumber').val(),
				email: jQuery('#email').val(),
				password: jQuery('#password').val()
			},
            beforeSend: function () {
        		//jQuery('#idButton').hide();
        		//jQuery('#idWait').show();
            },
            success: function (response) {
            	if (response != null) {
            		loadDataToForm(null, null, null, null, null, null, null, null);
					alert('Enregistré avec succès');
					widgetView('list');
				} else {
					//alert('L\'utilisateur à été enregistré avec succès');
					alert('Impossible d\'effectuer cet enregistrement');
				}
            },
            error: function () {
                
            },
            complete: function () {
        		jQuery('#table').bootstrapTable('refresh', {
                    url: contextPath + '/settings/customersadvisorsjson'
                });
            }
        });
	}
	
	function edit(id){
		jQuery.ajax({
            type: "POST",
            url: contextPath+"/settings/loadcustomeradvisor",
            cache: false,
            data: {id: id},
			success: function (response) {
            	if (response != null) {
            		loadDataToForm(response.id, response.branch.id, response.name, response.bday, response.phoneNumber, response.address, response.email, null);
            		widgetView('edit');
				} else {
					alert('Impossible de charger cet objet');
				}
            },
            error: function () {
                
            }
        });
	}
	
	function del(id){
		var r = confirm('Confirmer la suppression ?');
		if (r == true) {
			jQuery.ajax({
	            type: 'POST',
	            url: contextPath+'/settings/deletecustomeradvisor',
	            cache: false,
	            data: {id: id},
				success: function (response) {
	            	if (response.result == true) {
	            		alert('Supprimé avec succès');
					} else {
						alert('Impossible de supprimer cet objet');
					}
	            },
	            error: function () {
	                
	            },
	            complete: function () {
            		jQuery('#table').bootstrapTable('refresh', {
                        url: contextPath + '/settings/customersadvisorsjson'
                    });
	            }
	        });
		} else {
		    
		} 
	}
	
	/*function bankFormatter(branch) {
        return branch.bank.name;
    }*/
	
	function branchFormatter(branch) {
        return branch.name;
    }
	
	function optionFormatter(id, row, index) {
		var option = '<a onclick="edit(\''+row.id+'\')" data-toggle="modal" href="#" title="Modifier Conseiller Client [NOM : '+row.name+' ]"><span class="glyphicon glyphicon-file"></span></a>&nbsp;';
		option += '&nbsp;<a onclick="del(\''+row.id+'\')" data-toggle="modal" href="#" title="Suprimer Conseiller Client [NOM : '+row.namee+' ]"><span class="glyphicon glyphicon-trash"></span></a>';
        return option;
    }
</script>

<div class="widget">
	<div class="widgetbox">                        
	    <div class="headtitle">
	        <div class="btn-group">
	            <button data-toggle="dropdown" class="btn dropdown-toggle">Action <span class="caret"></span></button>
				<ul class="dropdown-menu">
	              	<li><a href="#" onclick="widgetView('list')">Listes</a></li>
	              	<li class="divider"></li>
	              	<li><a href="#" onclick="widgetView('new')">Nouveau</a></li>
	            </ul>
	        </div>
	        <h4 id="widgetTitle" class="widgettitle">Liste Conseillers Clients</h4>
	    </div>
	    <div id="tableWidget" class="widgetcontent">
	    	<table id="table" class="table table-info table-striped"
			       	data-toggle="table"
			       	data-click-to-select="true"
			       	data-single-select="true"
			       	data-sort-name="name" data-sort-order="desc"
			       	data-url="${pageContext.request.contextPath}/settings/customersadvisorsjson"
			       	data-side-pagination="server"
			       	data-pagination="true"
			       	data-page-list="[5, 10, 20, 50, 100, 200]"
			       	data-search="true">
			    <thead>
			        <tr>
			        	<!-- <th data-field="branch" data-formatter="bankFormatter" data-align="left" data-sortable="true">Banque</th> -->
			        	<th data-field="branch" data-formatter="branchFormatter" data-align="left" data-sortable="true">Agence</th>
			        	<th data-field="name" data-align="left" data-sortable="true">Nom</th>
			        	<th data-field="bday" data-align="center" data-sortable="true">Date de Naissance</th>
			        	<th data-field="address" data-align="left" data-sortable="true">Adresse</th>
			        	<th data-field="phoneNumber" data-align="center" data-sortable="true">T&eacute;l&eacute;phone</th>
			        	<th data-field="email" data-align="left" data-sortable="true">Email</th>
			        	<th data-field="id" data-formatter="optionFormatter" data-align="center" data-sortable="false">Options</th>
			        </tr>
			    </thead>
			</table>
	    </div><!--widgetcontent-->
	    
	    <div id="formWidget" class="widgetcontent">
	        
	            <div class="row">
                    <div class="form-group col-md-4">
                    	<label for="bin">Agence</label>
                        <select data-placeholder="Choisir une agence ..." style="width:300px" class="chzn-select" id="idBranch" tabindex="1"></select>
                    </div>
                </div><div class="row">
                    <div class="form-group col-md-4">
                    	<label for="bin">Nom</label>
                        <input type="text" class="form-control input-default" id="name" placeholder="Code Conseiller Client">
                    </div>
                    <div class="form-group col-md-4">
                    	<label for="bin">Date de Naissance</label>
                        <input type="text" class="form-control input-default" id="birthday" placeholder="Date de Naissance">
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-md-4">
                    	<label for="bin">Adresse</label>
                        <input type="text" class="form-control input-default" id="address" placeholder="Adresse">
                    </div>
                    <div class="form-group col-md-4">
                    	<label for="bin">T&eacute;l&eacute;phone</label>
                        <input type="text" class="form-control input-default" id="phoneNumber" placeholder="Telephone">
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-md-4">
                    	<label for="bin">Email</label>
                        <input type="text" class="form-control input-default" id="email" placeholder="Email">
                    </div>
                    <div class="form-group col-md-4">
                    	<label for="bin">Mot de Passe</label>
                        <input type="password" class="form-control input-default" id="password" placeholder="Mot de Passe">
                    </div>
                </div>
	            <input type="hidden" class="form-control input-default" id="id" placeholder="id">
	            
	            <p class="stdformbutton">
                    <button class="btn btn-primary" onclick="save()">Enregistrer</button>
                    <button id="reset" type="reset" class="btn btn-default">Annuler</button>
                </p>
	        
	    </div><!-- widgetcontent-->
    </div><!--widgetbox-->
</div><!-- widgetcontent-->